      ...
      >
        <div class="ui-pts-input__form">
          <UiFieldInput
            ...
            :wrapper-class="getFieldGridSpan(subField)"
          />
        </div>
...
import type { IObjectStepField, IUiObjectStepField } from '@/core/interface/Object'
...
watch(
  () => props.subFields,
  () => {
    updateFormFieldsFromSteps(props.subFields, true)
  },
  {
    immediate: true,
    deep: true,
...
const valueString = ref('Не задано')
const getFieldGridSpan = (field: IObjectStepField) => {
  return 'span-' + (field.Size || 2)
}
watch(
  () => props.value,
  (value) => {
    if (value) {
      ...
      if (!value?.CompanyName && !value?.Position) {
        valueString.value = 'Не задано'
      } else if (value?.CompanyName && !value?.Position) {
        valueString.value = value.CompanyName
      } else {
        valueString.value = `${value.CompanyName}, ${value.Position}`
      }
    }
  },
  {
...
useFieldInputEvents(props.name, FieldType.WorkingPlace, showSavedDetails)
</script>
